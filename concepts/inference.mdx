...
Optional query filters for `GET /inferences`: `limit`, `offset`, `model_id`, `task`, `project_id`, `training_job_id`, `latency_min`, `latency_max` (ms), `llmaj_score_min`, `llmaj_score_max` (0.0–1.0, LLM-as-judge score), `since`, `until` (ISO 8601 bounds on `created_at`).

`GET /inferences/INFERENCE_ID` also surfaces any automated LLM-as-judge verdict (`llmaj_score`, `llmaj_verdict`, `llmaj_reasoning`) and any human feedback already submitted (`human_verdict`, `human_corrected_output`, `human_feedback_notes`) inline on the record.