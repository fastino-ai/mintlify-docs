---
title: "Inference on Fastino: native, OpenAI, and Anthropic APIs"
description: "Run inference on Fastino via the native /inference endpoint, OpenAI-compatible chat completions, or Anthropic-compatible messages — all reach the same models."
hidden: true
noindex: true
---

Once you have a trained model — or want to use a base model directly — you run inference by sending a request to the Fastino API. The `model_id` field accepts either a base model ID (like `fastino/gliner2-base-v1`) or the job ID (a UUID) returned from a completed training job (like `3fa85f64-5717-4562-b3fc-2c963f66afa6`). Fastino routes the request to the right deployment automatically.

Fastino supports three request formats: its own native format, an OpenAI-compatible format, and an Anthropic-compatible format. All three reach the same underlying models, and all three accept your API key the same way: either an `X-API-Key` header or an `Authorization: Bearer <key>` header — whichever your SDK sends by default works, no per-format configuration needed.
...
## Fastino native format

Use `POST /inference` with the Fastino schema format. This is the most expressive option and gives you full control over extraction tasks.
...
Fastino exposes an OpenAI-compatible endpoint at `https://api.pioneer.ai/v1`. Point any existing OpenAI SDK or integration at this base URL and use your Fastino API key — no other changes required.
...
Fastino also exposes an Anthropic-compatible endpoint. Set your SDK's `base_url` to `https://api.pioneer.ai/v1` and use your Fastino API key in place of an Anthropic key — the Anthropic SDK sends it as `x-api-key`, which Fastino accepts the same as the other two formats.
