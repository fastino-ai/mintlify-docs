...
description: "The Pioneer Router picks a decoder model per request from Pioneer's supported families, using your configured quality-versus-cost policy and routing rules."
---
...
Configure the router from the Pioneer dashboard, then use the supported model
families listed in the live catalog. Routing details — selected model,
confidence, rule, and savings — are stored in
`inferences.metadata.model_routing` and visible in the Pioneer dashboard.
...
<ParamField path="fallback" default="claude-sonnet-5" type="string">
