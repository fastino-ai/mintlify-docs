...
Send `model: "pioneer/auto"` on Anthropic-compatible requests (or run `claude --model pioneer/auto` in [Claude Code](/claude-code)). Pioneer resolves the router choice per turn and returns `pioneer_routed_model` on the response so clients can surface which backend model ran.
