...
| `gpt-5.6-luna` | GPT-5.6 Luna | \$0.20 | \$1.20 | 1,050,000 |
| `gpt-5.6-terra` | GPT-5.6 Terra | \$2.00 | \$12.00 | 1,050,000 |
...
`thinkingmachines/inkling-small` is a multimodal model (12B active params) that also accepts image input, tuned for strong performance at low cost and latency.
...
| Qwen (qwen3.6 / qwen3.7 serverless models) | 0.2× input | 1.25× input |
| Mistral | input rate (no cache discount on most models; Mistral Small 4 reads at ~0.67× input) | input rate |
| MiniMax M3 | 0.2× input | input rate |
| NVIDIA (Nemotron 3 Ultra only — Nano and Super have no cache discount) | 0.25× input | input rate |
| xAI (Grok) | 0.25× input | input rate |
| Sakana AI | 0.1× input | input rate |

Self-hosted models served on Modal, Qwen3 models outside the qwen3.6/3.7 serverless family (including the Qwen training-table models and `Qwen/Qwen3-235B-A22B-Instruct-2507`), MiniMax M2.7, and any other model without an explicit cache discount bill cached input at the standard input rate. Fireworks-served models (DeepSeek, Kimi, GLM, MiMo) carry per-model cache rates rather than a single ratio, so query the live catalog for exact values.

For example, `claude-opus-4-8` lists \$5.50 input, so cache reads bill at \$0.55 and cache writes at \$6.875 per 1M tokens. `gpt-5.1` lists \$1.25 input, so cache reads bill at \$0.125 per 1M tokens.
