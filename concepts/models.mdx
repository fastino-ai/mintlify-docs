...
Fireworks-served models (DeepSeek, GLM, Kimi) carry per-model cache rates rather than a single ratio, so query the live catalog for exact values. Any other model without an explicit cache discount bills cached input at the standard input rate.
