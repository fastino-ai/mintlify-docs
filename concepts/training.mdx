...
Pioneer supports three post-training algorithms, selected with the `training_algorithm` parameter: supervised fine-tuning (`sft`, the default) and the reinforcement-learning methods GRPO (`grpo`) and DPO (`dpo`). See the [Post-training guide](/guides/post-training) for when to use each and how to format datasets.
...
| `training_algorithm` | No | `"sft"` (default), `"grpo"`, or `"dpo"`. RL algorithms require an `rl_config`. See [Post-training](/guides/post-training). |
