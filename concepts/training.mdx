...
description: "Understand how Fastino training jobs work — from submitting a job and polling status to reading metrics, stopping jobs, and downloading trained model weights."
hidden: true
noindex: true
---

Fine-tuning in Fastino adapts a base model to your specific task and domain using your labeled dataset. You submit a training job through the API, Fastino handles the compute, and you get back a trained model you can call for inference or download. The whole process is asynchronous — you start the job, then poll until it finishes.

Fastino uses supervised fine-tuning (`sft`) for all new training jobs. See the [LLM fine-tuning guide](/guides/fine-tune-llm) for dataset formatting and decoder training examples.
...
<Steps>
  <Step title="requested">
    Your job has been accepted and is queued for execution. Fastino is allocating compute.
  </Step>
  <Step title="running">
  ...
  <Step title="normalizing / artifact_ready">
    Intermediate post-training steps — Fastino is normalizing and packaging the trained artifact. You'll typically only see these transiently between `complete` and `deployed`.
  </Step>
  <Step title="deployed">
