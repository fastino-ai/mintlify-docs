...
Upload a labeled held-out set with `type: "evaluation"` on [`POST /felix/datasets/upload/url`](/concepts/datasets#uploading-your-own-dataset). Wait until the dataset status is `ready`, then pass its name as `dataset_name` below. Decoder evals use the same chat JSONL format as training (`dataset_type: "decoder"`). You can also flip an existing dataset's purpose with `PATCH /felix/datasets/{dataset_id}/type`.
