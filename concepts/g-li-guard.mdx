...
description: "Run GLiGuard, Fastino's open-source 300M safety moderation SLM, to classify prompts and completions for harmful, unsafe, or policy-violating content."
hidden: true
---
...
## Inference GLiGuard in Fastino

`POST /v1/chat/completions`

Runs GLiGuard over the supplied messages and returns a classification for each task defined in `schema`. Fastino exposes an OpenAI-compatible endpoint at `https://api.fastino.ai/v1`, so you call GLiGuard through the standard chat completions route using the model ID `fastino/gliguard-LLMGuardrails-300M`.
...
curl -X POST "https://api.fastino.ai/v1/chat/completions" \
  ...
  -H "Authorization: Bearer <YOUR_FASTINO_API_KEY>" \
  -d '{
