...
Runs GLiNER2-PII over the supplied messages and returns the entities defined in `schema`. Fastino exposes an OpenAI-compatible endpoint at `https://api.pioneer.ai/v1`, so you call GLiNER2-PII through the standard chat completions route using the model ID `fastino/gliner2-privacy-filter-PII-multi`.
