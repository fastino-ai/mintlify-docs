...
description: "Run GLiNER2-PII, Fastino's open-source multilingual PII detection SLM, to identify names, emails, phone numbers, and other personal data in text."
hidden: true
---
...
## Inference GLiNER2-PII in Fastino

`POST /v1/chat/completions`

Runs GLiNER2-PII over the supplied messages and returns the entities defined in `schema`. Fastino exposes an OpenAI-compatible endpoint at `https://api.fastino.ai/v1`, so you call GLiNER2-PII through the standard chat completions route using the model ID `fastino/gliner2-privacy-filter-PII-multi`.
