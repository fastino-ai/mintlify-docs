...
Fastino exposes two released GLiNER inference operations:

- `POST /v1/chat/completions` uses an OpenAI-compatible request envelope plus
  Fastino's top-level `schema` extension.
- `POST /v1/gliner-2` provides the native GLiNER request and response contract
  published in the [OpenAPI document](https://api.fastino.ai/openapi.json).

This page documents the `/v1/chat/completions` raw HTTP contract. It does not cover SDK
usage, model training, evaluation, inference history, or feedback.
