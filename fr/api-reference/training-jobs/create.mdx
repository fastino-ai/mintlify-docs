...
<ParamField body="base_model" type="string" required>
  Un ID de modèle entraînable issu de `GET /base-models?supports_training=true`, ou un UUID de checkpoint pris en charge.
</ParamField>
...
curl -X POST https://api.pioneer.ai/felix/training-jobs \
  ...
  -d '{
    ...
    "base_model": "MODEL_ID_FROM_BASE_MODELS",
...
Renvoie `200` avec l'enregistrement complet du job d'entraînement. Enregistrez son `id` et interrogez [`GET /felix/training-jobs/{job_id}`](/fr/api-reference/training-jobs/get).
