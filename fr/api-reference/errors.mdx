---
title: "Codes d'erreur et formats de réponse de l'API Fastino"
sidebarTitle: "Erreurs"
description: "Tous les codes 4xx et 5xx renvoyés par l'API Fastino, le format JSON par famille et comment résoudre les erreurs de facturation, de débit et de validation."
---
...
**Comment corriger :** Vérifiez à nouveau l'ID ou le nom dans le chemin ou le corps de la requête. Utilisez l'endpoint de liste `GET` correspondant (par exemple `GET /v1/felix/training-jobs`, `GET /v1/base-models`) pour confirmer que la ressource existe.
...
- Omission de `base_model` dans `POST /v1/felix/training-jobs`
- Transmission d'un `task_type` non pris en charge à `POST /v1/generate`
