---
title: "Gestion des clés API Fastino : créer, lister et révoquer"
sidebarTitle: "Clés API"
description: "Créez des clés API Fastino dans le tableau de bord, listez les clés actives et révoquez-les par programmation. La clé secrète n'apparaît qu'à la création."
---

Chaque requête à l'API Fastino nécessite une clé API transmise dans l'en-tête `X-API-Key`. Créez les clés dans le tableau de bord Fastino, puis utilisez les endpoints de gestion des clés pour lister et révoquer les clés existantes de manière programmatique.

<Tip>
  Stockez les clés API dans des variables d'environnement plutôt que de les coder en dur dans le code source. Par exemple, définissez `FASTINO_API_KEY` dans votre environnement et lisez-la à l'exécution. Ne validez jamais de clés API dans le contrôle de version.
</Tip>
...
Génère une nouvelle clé API associée à votre compte. Cet endpoint est utilisé par le tableau de bord Fastino et nécessite une session de navigateur. Les appels authentifiés avec une clé API existante sont rejetés avec `403 Forbidden` pour empêcher l'enchaînement des identifiants.
...
<ResponseField name="stripe_customer_created" type="boolean">
  Indique si Fastino a créé un enregistrement client Stripe lors de la création de la clé.
</ResponseField>
...
<ParamField body="key_id" type="string" required>
  L'ID unique de la clé à révoquer, tel que renvoyé par `GET /v1/list-api-keys`.
</ParamField>
