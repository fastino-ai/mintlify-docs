---
title: "Endpoints d'historique d'inférence et feedback Fastino"
sidebarTitle: "Historique & Feedback"
description: "Listez l'historique d'inférence Fastino, filtrez par modèle ou projet et envoyez des corrections qu'Adaptive Inference utilise pour réentraîner votre modèle."
hidden: true
noindex: true
---

Fastino stocke chaque appel d'inférence et vous permet de récupérer les résultats par ID ou en masse. Vous pouvez également soumettre un feedback de correction sur des inférences individuelles. Ce feedback signale ce que le modèle a mal fait et alimente l'Adaptive Inference, qui réentraîne automatiquement votre modèle sur des exemples corrigés issus du trafic en production.

## Endpoints

| Méthode | Chemin | Description |
|---|---|---|
| `GET` | `/v1/inferences` | Lister les inférences passées |
| `GET` | `/v1/inferences/:id` | Obtenir les détails d'une inférence |
| `POST` | `/v1/inferences/:id/feedback` | Soumettre un feedback |
| `GET` | `/v1/inferences/:id/feedback` | Obtenir le feedback stocké |

## Lister les inférences passées

`GET /v1/inferences` renvoie une liste paginée des appels d'inférence passés. Utilisez les paramètres de requête ci-dessous pour filtrer les résultats.
...
```bash cURL
curl "https://api.fastino.ai/v1/inferences?limit=20&offset=0&model_id=job_abc123" \
...
response = requests.get(
    "https://api.fastino.ai/v1/inferences",
...
`GET /v1/inferences/:id` renvoie l'enregistrement complet d'une seule inférence passée, y compris le texte d'entrée, le schéma, la réponse du modèle et l'horodatage.
...
response = requests.get(
    "https://api.fastino.ai/v1/inferences/INFERENCE_ID",
    headers={"X-API-Key": "YOUR_API_KEY"}
)
...
`POST /v1/inferences/:id/feedback` vous permet de marquer une inférence passée comme correcte ou incorrecte, en joignant éventuellement la sortie corrigée. Les verdicts incorrects accompagnés d'une correction sont utilisés comme exemples d'entraînement étiquetés pour l'Adaptive Inference.

<Note>
  Le feedback soumis ici alimente l'Adaptive Inference, la boucle d'amélioration continue de Fastino qui réentraîne automatiquement votre modèle sur les corrections collectées à partir du trafic en production. Consultez le [guide Adaptive Inference](/guides/adaptive-inference) pour les détails sur son fonctionnement.
</Note>
...
response = requests.post(
    "https://api.fastino.ai/v1/inferences/INFERENCE_ID/feedback",
    headers={
...
`GET /v1/inferences/:id/feedback` renvoie le feedback précédemment soumis pour une inférence spécifique. Renvoie `404` si aucun feedback n'a encore été soumis.
...
- [Inférence native Fastino](/api-reference/inference/pioneer) : exécuter de nouvelles inférences
