...
<Steps>
  ...
  <Step title="Lancez une tâche d'entraînement (optionnel)">
    ...
    La réponse inclut un `id` de tâche. Interrogez `GET /v1/training-jobs/{id}` pour vérifier le statut. Une fois la tâche à `complete`, utilisez l'ID de la tâche comme `model` dans les appels à `/v1/chat/completions`.
