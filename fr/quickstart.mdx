...
<Steps>
  ...
  <Step title="Listez les modèles de base disponibles">
    Avant d'exécuter une inférence, vous pouvez parcourir les modèles proposés par Fastino. Utilisez `GET /v1/base-models` pour consulter le catalogue complet. Passez `?supports_inference=true` pour filtrer sur les modèles que vous pouvez appeler immédiatement, ou `?task_type=decoder` pour n'afficher que les LLM.
  ...
  <Step title="Lancez une tâche d'entraînement (optionnel)">
    ...
    La réponse inclut un `id` de tâche. Interrogez `GET /v1/felix/training-jobs/{id}` pour vérifier le statut. Une fois la tâche à `complete`, utilisez l'ID de la tâche comme `model` dans les appels à `/v1/chat/completions`.
