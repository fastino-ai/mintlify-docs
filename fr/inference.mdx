...
- `POST /v1/chat/completions` utilise une enveloppe compatible OpenAI avec l’extension
  `schema` de premier niveau de Fastino.
- `POST /v1/gliner-2` fournit le contrat natif GLiNER publié dans le
  [document OpenAPI](https://api.fastino.ai/openapi.json).

Cette page documente le contrat HTTP brut de `/v1/chat/completions`. Elle ne couvre pas
l'utilisation du SDK, l'entraînement des modèles, l'évaluation, l'historique d'inférence
ni le feedback.
