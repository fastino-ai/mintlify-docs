...
Pioneer prend en charge trois algorithmes de post-entraînement, sélectionnés avec le paramètre `training_algorithm` : le fine-tuning supervisé (`sft`, la valeur par défaut) et les méthodes d'apprentissage par renforcement GRPO (`grpo`) et DPO (`dpo`). Consultez le [guide de fine-tuning LLM](/guides/fine-tune-llm) pour savoir quand utiliser chacun et comment formater les datasets.
...
| `training_algorithm` | Non | `"sft"` (par défaut), `"grpo"` ou `"dpo"`. Les algorithmes RL nécessitent un `rl_config`. Voir [Fine-tuning LLM](/guides/fine-tune-llm). |
