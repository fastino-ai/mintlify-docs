...
Une tâche peut également se terminer par **`errored`** (une erreur est survenue pendant l'entraînement), **`stopped`** (vous l'avez arrêtée proprement avec `POST /v1/training-jobs/:id/stop`, les checkpoints sont conservés), **`terminated`** (vous avez appelé `POST /v1/training-jobs/:id/terminate`, qui arrête la tâche *et* supprime définitivement ses checkpoints, action irréversible), ou **`paused`**.
...
|---|---|---|
| `POST` | `/v1/training-jobs` | Démarrer une nouvelle tâche d'entraînement |
| `GET` | `/v1/training-jobs` | Lister les tâches d'entraînement (filtrer par `project_id`, `status` ; paginer avec `limit`/`offset`) |
| `GET` | `/v1/training-jobs/:id` | Obtenir le statut et les métriques d'une tâche |
| `GET` | `/v1/training-jobs/:id/logs` | Obtenir les entrées de log d'entraînement structurées |
| `GET` | `/v1/training-jobs/:id/checkpoints` | Lister les checkpoints enregistrés |
| `POST` | `/v1/training-jobs/:id/checkpoints/:checkpoint_id/deploy` | Déployer un checkpoint spécifique pour l'inférence |
| `GET` | `/v1/training-jobs/:id/download` | Obtenir une URL présignée pour télécharger les poids entraînés (plan Pro et supérieur) |
| `POST` | `/v1/training-jobs/:id/stop` | Arrêter proprement une tâche en cours, en préservant les checkpoints |
| `POST` | `/v1/training-jobs/:id/terminate` | Arrêter la tâche et supprimer définitivement ses checkpoints (irréversible) |
| `DELETE` | `/v1/training-jobs/:id` | Supprimer l'enregistrement de la tâche (l'arrête aussi si elle est active et supprime ses checkpoints) |
