---
title: "Tâches d'entraînement Fastino : cycle de vie et poids"
sidebarTitle: "Entraînement"
description: "Fonctionnement des tâches d'entraînement Fastino : soumettez une tâche, interrogez son statut, lisez les métriques, arrêtez la tâche et téléchargez les poids."
---

Le fine-tuning dans Fastino adapte un modèle de base à votre tâche et à votre domaine spécifiques en utilisant votre dataset étiqueté. Vous soumettez une tâche d'entraînement via l'API, Fastino gère le calcul, et vous récupérez un modèle entraîné que vous pouvez appeler pour l'inférence ou télécharger. L'ensemble du processus est asynchrone : vous démarrez la tâche, puis vous interrogez son statut jusqu'à ce qu'elle se termine.

Les nouvelles tâches d'entraînement Fastino utilisent le fine-tuning supervisé (SFT). Consultez le [guide de fine-tuning LLM](/guides/fine-tune-llm) pour le format des datasets et des exemples d'entraînement de décodeurs.
...
<Steps>
  <Step title="requested">
    Votre tâche a été acceptée et est mise en file d'attente pour exécution. Fastino alloue les ressources de calcul.
  </Step>
  <Step title="running">
  ...
  <Step title="normalizing / artifact_ready">
    Étapes intermédiaires post-entraînement. Fastino normalise et empaquette l'artefact entraîné. Vous ne verrez généralement ces états que de manière transitoire entre `complete` et `deployed`.
  </Step>
  <Step title="deployed">
...
Une tâche peut également se terminer par **`errored`** (une erreur est survenue pendant l'entraînement), **`stopped`** (vous l'avez arrêtée proprement avec `POST /v1/felix/training-jobs/:id/stop`, les checkpoints sont conservés), **`terminated`** (vous avez appelé `POST /v1/felix/training-jobs/:id/terminate`, qui arrête la tâche *et* supprime définitivement ses checkpoints, action irréversible), ou **`paused`**.
...
| `base_model` | Oui | L'ID du modèle à fine-tuner. Utilisez une valeur issue de `GET /v1/base-models` ou un UUID de checkpoint d'une tâche précédente. |
...
Fastino enregistre des checkpoints pendant l'entraînement. Vous pouvez les lister à tout moment après le démarrage de la tâche :
...
|---|---|---|
| `POST` | `/v1/felix/training-jobs` | Démarrer une nouvelle tâche d'entraînement |
| `GET` | `/v1/felix/training-jobs` | Lister les tâches d'entraînement (filtrer par `project_id`, `status` ; paginer avec `limit`/`offset`) |
| `GET` | `/v1/felix/training-jobs/:id` | Obtenir le statut et les métriques d'une tâche |
| `GET` | `/v1/felix/training-jobs/:id/logs` | Obtenir les entrées de log d'entraînement structurées |
| `GET` | `/v1/felix/training-jobs/:id/checkpoints` | Lister les checkpoints enregistrés |
| `POST` | `/v1/felix/training-jobs/:id/checkpoints/:checkpoint_id/deploy` | Déployer un checkpoint spécifique pour l'inférence |
| `GET` | `/v1/felix/training-jobs/:id/download` | Obtenir une URL présignée pour télécharger les poids entraînés (plan Pro et supérieur) |
| `POST` | `/v1/felix/training-jobs/:id/stop` | Arrêter proprement une tâche en cours, en préservant les checkpoints |
| `POST` | `/v1/felix/training-jobs/:id/terminate` | Arrêter la tâche et supprimer définitivement ses checkpoints (irréversible) |
| `DELETE` | `/v1/felix/training-jobs/:id` | Supprimer l'enregistrement de la tâche (l'arrête aussi si elle est active et supprime ses checkpoints) |
