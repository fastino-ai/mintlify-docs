...
Fastino expose également un endpoint compatible Anthropic. Définissez le `base_url` de votre SDK sur `https://api.fastino.ai/v1` et utilisez votre clé API Fastino à la place d'une clé Anthropic. Le SDK Anthropic l'envoie en tant que `x-api-key`, que Fastino accepte de la même façon que les deux autres formats.
