---
title: "Inférence sur Fastino : API natives, OpenAI et Anthropic"
description: "Exécutez l'inférence sur Fastino via l'endpoint natif /inference, les chat completions compatibles OpenAI ou les messages compatibles Anthropic."
---

Une fois que vous disposez d'un modèle entraîné, ou que vous souhaitez utiliser directement un modèle de base, vous exécutez l'inférence en envoyant une requête à l'API Fastino. Le champ `model_id` accepte soit un ID de modèle de base (comme `fastino/gliner2-base-v1`), soit l'ID de tâche (un UUID) renvoyé par une tâche d'entraînement terminée (comme `3fa85f64-5717-4562-b3fc-2c963f66afa6`). Fastino route automatiquement la requête vers le bon déploiement.

Fastino prend en charge trois formats de requête : son propre format natif, un format compatible OpenAI et un format compatible Anthropic. Les trois accèdent aux mêmes modèles sous-jacents, et les trois acceptent votre clé API de la même façon : soit un en-tête `X-API-Key`, soit un en-tête `Authorization: Bearer <key>`. Celui que votre SDK envoie par défaut fonctionne, aucune configuration par format n'est nécessaire.
...
## Format natif Fastino

Utilisez `POST /inference` avec le format de schéma Fastino. C'est l'option la plus expressive et elle vous donne un contrôle complet sur les tâches d'extraction.
...
Fastino expose un endpoint compatible OpenAI à `https://api.pioneer.ai/v1`. Pointez n'importe quel SDK ou intégration OpenAI existant vers cette URL de base et utilisez votre clé API Fastino. Aucune autre modification n'est nécessaire.
...
Fastino expose également un endpoint compatible Anthropic. Définissez le `base_url` de votre SDK sur `https://api.pioneer.ai/v1` et utilisez votre clé API Fastino à la place d'une clé Anthropic. Le SDK Anthropic l'envoie en tant que `x-api-key`, que Fastino accepte de la même façon que les deux autres formats.
