...
description: "Exécutez l'inférence sur Fastino via les chat completions compatibles OpenAI ou les messages compatibles Anthropic."
---

Une fois que vous disposez d'un modèle entraîné, ou que vous souhaitez utiliser directement un modèle de base, envoyez son ID de modèle
ou l'UUID d'une tâche d'entraînement terminée dans le champ `model`. Fastino route automatiquement la requête vers le bon
déploiement.

Fastino expose des interfaces compatibles OpenAI et compatibles Anthropic. Pour l'extraction et la classification
GLiNER, utilisez `POST /v1/chat/completions` avec `model`, `messages` et
l'extension `schema` de premier niveau de Fastino. Consultez l'[API d'inférence](/fr/inference) pour les formes
canoniques de requête et de réponse.

<Note>
  Les endpoints de type chat (`/v1/chat/completions`, `/v1/responses`, `/v1/messages`) rejettent les requêtes vers un modèle de base décodeur pré-entraîné (non-instruct) avec un `400`. Utilisez plutôt la variante `-Instruct` du modèle.
</Note>
...
Fastino expose également un endpoint compatible Anthropic. Définissez le `base_url` de votre SDK sur `https://api.fastino.ai` et utilisez votre clé API Fastino à la place d'une clé Anthropic. Le SDK ajoute `/v1/messages` et envoie la clé en tant que `x-api-key`, que Fastino accepte.
...
Les endpoints compatibles OpenAI et Anthropic prennent tous deux en charge le streaming
(`stream: true`).
...
`store: false` est pris en charge sur `/v1/chat/completions` et `/v1/messages` et fonctionne
de manière identique pour les requêtes en streaming et non-streaming.
