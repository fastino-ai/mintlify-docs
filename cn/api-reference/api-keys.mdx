---
title: "Fastino API 密钥管理:创建、列出与撤销"
sidebarTitle: "API 密钥"
description: "在控制面板中创建 Fastino API 密钥,然后以编程方式列出和撤销密钥。完整的密钥仅在创建时返回一次,请立即将其存储在密钥管理服务中。"
---

Fastino API 的每个请求都需要在 `X-API-Key` 头中传入 API 密钥。请在 Fastino 控制面板中创建密钥,然后使用密钥管理端点以编程方式列出和撤销现有密钥。

<Tip>
  请将 API 密钥存储在环境变量中,而不是硬编码在源代码里。例如,在环境中设置 `FASTINO_API_KEY`,并在运行时读取它。切勿将 API 密钥提交到版本控制系统。
</Tip>
...
生成与您账户关联的新 API 密钥。此端点由 Fastino 控制面板使用,需要浏览器会话。使用现有 API 密钥进行身份验证的调用会被拒绝并返回 `403 Forbidden`,以防止凭证链式生成。
...
<ResponseField name="stripe_customer_created" type="boolean">
  Fastino 是否在创建密钥时创建了 Stripe 客户记录。
</ResponseField>
...
<ParamField body="key_id" type="string" required>
  要撤销的密钥的唯一 ID,由 `GET /v1/list-api-keys` 返回。
</ParamField>
