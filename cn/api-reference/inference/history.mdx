---
title: "Fastino 上的推理历史与反馈端点"
sidebarTitle: "历史与反馈"
description: "列出 Fastino 推理历史,按模型或项目过滤,获取单条结果,并提交更正以通过 Adaptive Inference 改进您的模型。"
hidden: true
noindex: true
---

Fastino 会存储每一次推理调用，您可以按 ID 或批量获取结果。您还可以针对单条推理提交更正反馈。这些反馈标示出模型出错的地方，并驱动 Adaptive Inference，它会基于来自线上流量的已更正样本自动重新训练您的模型。

## 端点

| Method | Path | 说明 |
|---|---|---|
| `GET` | `/v1/inferences` | 列出过往推理 |
| `GET` | `/v1/inferences/:id` | 获取推理详情 |
| `POST` | `/v1/inferences/:id/feedback` | 提交反馈 |
| `GET` | `/v1/inferences/:id/feedback` | 获取已存储的反馈 |

## 列出过往推理

`GET /v1/inferences` 返回过往推理调用的分页列表。使用下方的查询参数过滤结果。
...
`GET /v1/inferences/:id` 返回单条过往推理的完整记录，包括输入文本、schema、模型响应和时间戳。
...
`POST /v1/inferences/:id/feedback` 允许您将一条过往推理标记为正确或不正确，并可选择附上更正后的输出。带有更正内容的“不正确”判定会被用作 Adaptive Inference 的标注训练样本。

<Note>
  在此提交的反馈会驱动 Adaptive Inference。这是 Fastino 的持续改进闭环，会基于从线上流量收集到的更正自动重新训练您的模型。有关工作原理的详情，请参阅 [Adaptive Inference 指南](/guides/adaptive-inference)。
</Note>
...
`GET /v1/inferences/:id/feedback` 返回之前针对特定推理提交的反馈。如果尚未提交任何反馈，则返回 `404`。
...
- [Fastino 原生推理](/api-reference/inference/pioneer)：运行新的推理
