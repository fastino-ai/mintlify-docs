...
<ParamField body="base_model" type="string" required>
  来自 `GET /base-models?supports_training=true` 的可训练模型 ID，或受支持的检查点 UUID。
</ParamField>
...
返回 `200` 及完整的训练任务记录。请保存其 `id` 并轮询 [`GET /felix/training-jobs/{job_id}`](/cn/api-reference/training-jobs/get)。
