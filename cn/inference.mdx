...
Fastino 提供两种已发布的 GLiNER 推理操作：

- `POST /v1/chat/completions` 采用与 OpenAI 兼容的请求信封，
  并附加 Fastino 的顶层 `schema` 扩展。
- `POST /v1/gliner-2` 提供原生 GLiNER 请求与响应契约，
  该契约发布在 [OpenAPI 文档](https://api.fastino.ai/openapi.json)中。

本页记录 `/v1/chat/completions` 的原始 HTTP 契约。不涉及 SDK 使用、模型训练、评估、
推理历史或反馈。
