---
title: "Fastino 上的推理：原生、OpenAI 与 Anthropic API"
description: "在 Fastino 上运行推理:通过原生 /inference 端点、兼容 OpenAI 的 chat completions 或兼容 Anthropic 的 messages 接口调用相同的解码器和编码器模型,支持 GLiNER、Nemotron、Claude 与 DeepSeek 系列。"
---

有了训练完成的模型（或希望直接使用基础模型）之后，您可以向 Fastino API 发送请求来运行推理。`model_id` 字段既接受基础模型 ID（如 `fastino/gliner2-base-v1`），也接受已完成的训练任务返回的任务 ID（一个 UUID，例如 `3fa85f64-5717-4562-b3fc-2c963f66afa6`）。Fastino 会自动把请求路由到正确的部署。

Fastino 支持三种请求格式：自有的原生格式、兼容 OpenAI 的格式，以及兼容 Anthropic 的格式。三者都指向同一批底层模型，且都以相同方式接受您的 API 密钥：`X-API-Key` 请求头或 `Authorization: Bearer <key>` 请求头均可，您的 SDK 默认发送哪一种都能工作，无需按格式做额外配置。
...
使用 `POST /inference` 搭配 Fastino 的 schema 格式。这是最具表达力的选项，能让您对提取任务拥有完整控制。
...
Fastino 在 `https://api.pioneer.ai/v1` 提供一个兼容 OpenAI 的端点。将任何现有的 OpenAI SDK 或集成的 base URL 指向该地址，并使用您的 Fastino API 密钥，无需任何其他修改。
...
Fastino 也提供一个兼容 Anthropic 的端点。将 SDK 的 `base_url` 设置为 `https://api.pioneer.ai/v1`，并用 Fastino API 密钥代替 Anthropic 密钥。Anthropic SDK 会以 `x-api-key` 头发送它，Fastino 与其他两种格式一样能接受。
