...
Pioneer 支持三种训练后算法，通过 `training_algorithm` 参数选择：监督微调（`sft`，默认）以及强化学习方法 GRPO（`grpo`）和 DPO（`dpo`）。何时使用哪种以及数据集应如何组织，请参见 [LLM 微调指南](/guides/fine-tune-llm)。
...
| `training_algorithm` | 否 | `"sft"`（默认）、`"grpo"` 或 `"dpo"`。RL 算法需要 `rl_config`。参见 [LLM 微调](/guides/fine-tune-llm)。 |
