---
title: "Fastino-API-Schlüssel verwalten"
sidebarTitle: "API Keys"
description: "Fastino-API-Schlüssel im Dashboard erstellen sowie aktive Schlüssel programmatisch auflisten und widerrufen. Der Secret Key erscheint nur bei der Erstellung."
---

Jede Anfrage an die Fastino API erfordert einen API-Schlüssel, der im `X-API-Key`-Header übergeben wird. Erstellen Sie Schlüssel im Fastino-Dashboard und verwenden Sie anschließend die Endpunkte zur Schlüsselverwaltung, um bestehende Schlüssel programmatisch aufzulisten und zu widerrufen.

<Tip>
  Speichern Sie API-Schlüssel in Umgebungsvariablen, statt sie fest im Quellcode zu hinterlegen. Legen Sie beispielsweise `FASTINO_API_KEY` in Ihrer Umgebung fest und lesen Sie den Wert zur Laufzeit ein. Committen Sie API-Schlüssel niemals in die Versionskontrolle.
</Tip>
...
Erstellt einen neuen API-Schlüssel, der Ihrem Konto zugeordnet ist. Dieser Endpunkt wird vom Fastino-Dashboard verwendet und erfordert eine Browser-Sitzung. Mit einem bestehenden API-Schlüssel authentifizierte Aufrufe werden mit `403 Forbidden` abgelehnt, um Credential Chaining zu verhindern.
...
<ResponseField name="stripe_customer_created" type="boolean">
  Gibt an, ob Fastino während der Schlüsselerstellung einen Stripe-Kundendatensatz angelegt hat.
</ResponseField>
...
<ParamField body="key_id" type="string" required>
  Die eindeutige ID des zu widerrufenden Schlüssels, wie sie von `GET /v1/list-api-keys` zurückgegeben wird.
</ParamField>
