---
title: "Inferenz-Historie- und Feedback-Endpunkte auf Fastino"
sidebarTitle: "Historie & Feedback"
description: "Rufen Sie die Fastino-Inferenz-Historie ab, filtern Sie nach Modell oder Projekt und senden Sie Korrekturen, die Adaptive Inference zum Nachtrainieren nutzt."
hidden: true
noindex: true
---

Fastino speichert jeden Inferenzaufruf und lässt Sie Ergebnisse per ID oder gesammelt abrufen. Sie können außerdem Korrektur-Feedback zu einzelnen Inferenzen einreichen. Dieses Feedback signalisiert, was das Modell falsch gemacht hat, und treibt Adaptive Inference an, das Ihr Modell automatisch mit korrigierten Beispielen aus dem Live-Traffic nachtrainiert.

## Endpunkte

| Method | Path | Beschreibung |
|---|---|---|
| `GET` | `/v1/inferences` | Vergangene Inferenzen auflisten |
| `GET` | `/v1/inferences/:id` | Inferenzdetails abrufen |
| `POST` | `/v1/inferences/:id/feedback` | Feedback einreichen |
| `GET` | `/v1/inferences/:id/feedback` | Gespeichertes Feedback abrufen |

## Vergangene Inferenzen auflisten

`GET /v1/inferences` gibt eine paginierte Liste vergangener Inferenzaufrufe zurück. Verwenden Sie die folgenden Query-Parameter, um die Ergebnisse zu filtern.
...
`GET /v1/inferences/:id` gibt den vollständigen Record für eine einzelne vergangene Inferenz zurück, einschließlich Eingabetext, Schema, Modellantwort und Zeitstempel.
...
Mit `POST /v1/inferences/:id/feedback` können Sie eine vergangene Inferenz als korrekt oder inkorrekt markieren und optional die korrigierte Ausgabe anhängen. Inkorrekt-Verdicts mit einer Korrektur werden als gelabelte Trainingsbeispiele für Adaptive Inference verwendet.

<Note>
  Das hier eingereichte Feedback treibt Adaptive Inference an. Das ist Fastinos kontinuierliche Verbesserungsschleife, die Ihr Modell automatisch mit Korrekturen nachtrainiert, die aus dem Live-Traffic gesammelt wurden. Details zur Funktionsweise finden Sie im [Adaptive-Inference-Leitfaden](/guides/adaptive-inference).
</Note>
...
`GET /v1/inferences/:id/feedback` gibt das Feedback zurück, das zuvor für eine bestimmte Inferenz eingereicht wurde. Gibt `404` zurück, wenn noch kein Feedback eingereicht wurde.
...
- [Fastino native Inferenz](/api-reference/inference/pioneer): neue Inferenzen ausführen
