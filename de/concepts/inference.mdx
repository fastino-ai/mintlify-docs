...
Sobald Sie ein trainiertes Modell haben oder ein Basismodell direkt verwenden möchten, senden Sie dessen Modell-ID
oder die UUID eines abgeschlossenen Trainingsjobs im Feld `model`. Fastino leitet die Anfrage automatisch an das richtige
Deployment weiter.

Fastino stellt OpenAI-kompatible und Anthropic-kompatible Schnittstellen bereit. Für GLiNER-Extraktion
und -Klassifikation verwenden Sie `POST /v1/chat/completions` mit `model`, `messages` und
Fastinos Top-Level-Erweiterung `schema`. Die maßgeblichen Anfrage- und Antwortstrukturen finden Sie in der [Inferenz-API](/de/inference).

<Note>
  Die Chat-förmigen Endpunkte (`/v1/chat/completions`, `/v1/responses`, `/v1/messages`) lehnen Anfragen für ein vortrainiertes (nicht-instruct) Decoder-Basismodell mit `400` ab. Verwenden Sie stattdessen die `-Instruct`-Variante des Modells.
</Note>
...
Fastino stellt außerdem einen Anthropic-kompatiblen Endpunkt bereit. Setzen Sie die `base_url` Ihres SDKs auf `https://api.fastino.ai` und verwenden Sie Ihren Fastino-API-Key anstelle eines Anthropic-Keys. Das SDK hängt `/v1/messages` an und sendet den Key als `x-api-key`, was Fastino akzeptiert.
...
Sowohl die OpenAI-kompatiblen als auch die Anthropic-kompatiblen Endpunkte unterstützen Streaming
(`stream: true`).
...
`store: false` wird auf `/v1/chat/completions` und `/v1/messages` unterstützt und funktioniert
für Streaming- und Nicht-Streaming-Anfragen identisch.
