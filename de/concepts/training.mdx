...
|---|---|---|
| `POST` | `/v1/training-jobs` | Einen neuen Trainingsjob starten |
| `GET` | `/v1/training-jobs` | Trainingsjobs auflisten (nach `project_id`, `status` filtern; mit `limit`/`offset` paginieren) |
| `GET` | `/v1/training-jobs/:id` | Job-Status und Metriken abrufen |
| `GET` | `/v1/training-jobs/:id/logs` | Strukturierte Trainings-Log-Einträge abrufen |
| `GET` | `/v1/training-jobs/:id/checkpoints` | Gespeicherte Checkpoints auflisten |
| `POST` | `/v1/training-jobs/:id/checkpoints/:checkpoint_id/deploy` | Einen bestimmten Checkpoint für Inferenz deployen |
| `GET` | `/v1/training-jobs/:id/download` | Eine Presigned-URL zum Herunterladen trainierter Gewichte abrufen (Pro-Plan+) |
| `POST` | `/v1/training-jobs/:id/stop` | Einen laufenden Job sauber stoppen und Checkpoints bewahren |
| `POST` | `/v1/training-jobs/:id/terminate` | Den Job stoppen und seine Checkpoints permanent löschen (unumkehrbar) |
| `DELETE` | `/v1/training-jobs/:id` | Den Job-Record löschen. Stoppt ihn auch, wenn er aktiv ist, und löscht seine Checkpoints |
