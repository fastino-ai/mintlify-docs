---
title: "Fastino-Trainingsjobs: Lebenszyklus, Metriken und Gewichte"
sidebarTitle: "Training"
description: "So funktionieren Fastino-Trainingsjobs: Job absenden, Status abfragen, Metriken lesen, Job stoppen und trainierte Modellgewichte herunterladen."
---

Fine-Tuning in Fastino passt ein Basismodell mithilfe Ihres gelabelten Datasets an Ihre spezifische Aufgabe und Domäne an. Sie senden einen Trainingsjob über die API, Fastino übernimmt die Compute-Ressourcen, und Sie erhalten ein trainiertes Modell zurück, das Sie für Inferenz aufrufen oder herunterladen können. Der gesamte Prozess ist asynchron. Sie starten den Job und fragen dann ab, bis er abgeschlossen ist.

Neue Trainingsjobs in Fastino verwenden Supervised Fine-Tuning (SFT). Informationen zu Dataset-Formaten und Decoder-Trainingsbeispielen finden Sie im [LLM-Fine-Tuning-Leitfaden](/guides/fine-tune-llm).
...
<Steps>
  <Step title="requested">
    Ihr Job wurde angenommen und ist zur Ausführung eingereiht. Fastino allokiert Compute-Ressourcen.
  </Step>
  <Step title="running">
  ...
  <Step title="normalizing / artifact_ready">
    Post-Training-Zwischenschritte. Fastino normalisiert und paketiert das trainierte Artefakt. Diese Zustände sehen Sie typischerweise nur kurz zwischen `complete` und `deployed`.
  </Step>
  <Step title="deployed">
...
| `base_model` | Ja | Die Modell-ID, die feinabgestimmt werden soll. Verwenden Sie einen Wert aus `GET /v1/base-models` oder eine Checkpoint-UUID aus einem früheren Job. |
...
Fastino speichert während des Trainings Checkpoints. Sie können sie jederzeit nach dem Start des Jobs auflisten:
...
|---|---|---|
| `POST` | `/v1/felix/training-jobs` | Einen neuen Trainingsjob starten |
| `GET` | `/v1/felix/training-jobs` | Trainingsjobs auflisten (nach `project_id`, `status` filtern; mit `limit`/`offset` paginieren) |
| `GET` | `/v1/felix/training-jobs/:id` | Job-Status und Metriken abrufen |
| `GET` | `/v1/felix/training-jobs/:id/logs` | Strukturierte Trainings-Log-Einträge abrufen |
| `GET` | `/v1/felix/training-jobs/:id/checkpoints` | Gespeicherte Checkpoints auflisten |
| `POST` | `/v1/felix/training-jobs/:id/checkpoints/:checkpoint_id/deploy` | Einen bestimmten Checkpoint für Inferenz deployen |
| `GET` | `/v1/felix/training-jobs/:id/download` | Eine Presigned-URL zum Herunterladen trainierter Gewichte abrufen (Pro-Plan+) |
| `POST` | `/v1/felix/training-jobs/:id/stop` | Einen laufenden Job sauber stoppen und Checkpoints bewahren |
| `POST` | `/v1/felix/training-jobs/:id/terminate` | Den Job stoppen und seine Checkpoints permanent löschen (unumkehrbar) |
| `DELETE` | `/v1/felix/training-jobs/:id` | Den Job-Record löschen. Stoppt ihn auch, wenn er aktiv ist, und löscht seine Checkpoints |
