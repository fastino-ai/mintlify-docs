...
Pioneer unterstützt drei Post-Training-Algorithmen, ausgewählt über den Parameter `training_algorithm`: Supervised Fine-Tuning (`sft`, Standard) sowie die Reinforcement-Learning-Methoden GRPO (`grpo`) und DPO (`dpo`). Im [LLM-Fine-Tuning-Leitfaden](/guides/fine-tune-llm) erfahren Sie, wann Sie welche Methode einsetzen und wie Sie Datasets formatieren.
...
| `training_algorithm` | Nein | `"sft"` (Standard), `"grpo"` oder `"dpo"`. RL-Algorithmen erfordern ein `rl_config`. Siehe [LLM-Fine-Tuning](/guides/fine-tune-llm). |
