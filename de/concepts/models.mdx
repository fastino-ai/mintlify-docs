...
Diese LLMs sind für LoRA-Fine-Tuning über `POST /felix/training-jobs` verfügbar. Wenn Sie einen Job einreichen, leitet Pioneer ihn automatisch an den besten verfügbaren Anbieter weiter.
...
Diese Decoder-Modelle sind vorab bereitgestellt und sofort für Inferenz verfügbar. Trainierbare Decoder-Zeilen, die auch Serverless-Inferenz unterstützen, sind in der obigen Trainingstabelle aufgeführt und werden hier nicht wiederholt.
...
`poolside/laguna-s-2.1` ist ein Open-Weight-Modell (OpenMDW-1.1-Lizenz) für agentisches Coding und langfristige Arbeit.
...
`thinkingmachines/inkling-small` ist ein multimodales Modell (12B aktive Parameter), das auch Bildeingaben akzeptiert und für starke Leistung bei niedrigen Kosten und geringer Latenz abgestimmt ist.
...
| OpenAI (GPT-4-Familie) | 0,5× Input | Input-Rate |
| OpenAI (GPT-5-Familie) | 0,1× Input | Input-Rate (1,25× bei GPT-5.6 Luna/Sol/Terra) |
| Google (Gemini) | 0,1× Input | Input-Rate |
| Qwen (qwen3.6- / qwen3.7-Serverless-Modelle) | 0,2× Input | 1,25× Input |
| Mistral | Input-Rate (kein Cache-Rabatt bei den meisten Modellen; Mistral Small 4 liest bei ~0,67× Input) | Input-Rate |
| MiniMax M3 | 0,2× Input | Input-Rate |
| NVIDIA (nur Nemotron 3 Ultra – Nano und Super haben keinen Cache-Rabatt) | 0,25× Input | Input-Rate |
| xAI (Grok) | 0,25× Input | Input-Rate |
| Sakana AI | 0,1× Input | Input-Rate |

Auf Modal gehostete selbst-gehostete Modelle, Qwen3-Modelle außerhalb der qwen3.6/3.7-Serverless-Familie (einschließlich der Qwen-Modelle aus der Trainingstabelle und `Qwen/Qwen3-235B-A22B-Instruct-2507`), MiniMax M2.7 und alle anderen Modelle ohne expliziten Cache-Rabatt rechnen gecachten Input zur regulären Input-Rate ab. Über Fireworks bereitgestellte Modelle (DeepSeek, Kimi, GLM, MiMo) haben Cache-Raten pro Modell statt eines einzelnen Verhältnisses, fragen Sie also den Live-Katalog für exakte Werte ab.

Zum Beispiel listet `claude-opus-4-8` \$5,50 Input, sodass Cache-Reads mit \$0,55 und Cache-Writes mit \$6,875 pro 1 Mio. Tokens abgerechnet werden. `gpt-5.1` listet \$1,25 Input, sodass Cache-Reads mit \$0,125 pro 1 Mio. Tokens abgerechnet werden.
