...
Fastino stellt zwei veröffentlichte GLiNER-Inferenzoperationen bereit:

- `POST /v1/chat/completions` verwendet einen OpenAI-kompatiblen Anfrage-Umschlag sowie
  Fastinos `schema`-Erweiterung auf oberster Ebene.
- `POST /v1/gliner-2` bietet den nativen GLiNER-Anfrage- und Antwortvertrag, der im
  [OpenAPI-Dokument](https://api.fastino.ai/openapi.json) veröffentlicht ist.

Diese Seite dokumentiert den rohen HTTP-Vertrag von `/v1/chat/completions`. Sie behandelt weder
die SDK-Nutzung noch Modelltraining, Evaluation, Inferenz-Historie oder Feedback.
