---
title: "Claude Code"
description: "Point Claude Code at Fastino for multi-model inference with router-backed pioneer/auto routing, model discovery, and full /model picker support inside the CLI."
---

Fastino exposes an [Anthropic-compatible API](/api-reference/inference/anthropic-compatible). Claude Code can use it like a custom gateway: set `ANTHROPIC_BASE_URL`, authenticate with your Fastino API key, and switch models with `/model` or `pioneer/auto`.
...
<Steps>
  ...
  <Step title="Persist Fastino env vars">
    Create `~/.pioneer/env` with these contents:

    ```bash
    unset ANTHROPIC_AUTH_TOKEN CLAUDE_CODE_OAUTH_TOKEN
    export ANTHROPIC_API_KEY="<YOUR_FASTINO_API_KEY>"
    export ANTHROPIC_BASE_URL="https://api.fastino.ai"
  ...
  <Step title="Clear stale Claude.ai auth">
    Sign out of Claude.ai so Claude Code uses your Fastino key:

    ```bash
    claude auth logout >/dev/null 2>&1 || true
    ```

    If Claude Code previously asked whether to trust this API key and you chose **No**, move the key tail from `customApiKeyResponses.rejected` to `approved` in `~/.claude.json`, or rerun setup from the Fastino dashboard integration guide (it repairs this automatically).
  </Step>
  <Step title="Install the routed-savings hook">
    When you use `pioneer/auto`, Fastino stamps `pioneer_savings` on each response — the per-1M-token price difference between the model the router picked and a frontier reference model. Claude Code does not surface that by default, so install a **Stop** hook (Claude Code's end-of-turn hook) that multiplies those rate differences by your actual token usage, sums them across the session, and ends each turn with a line like:

    ```text
    Fastino routing saved ~$1.43 this session (vs claude-opus-4-7)
    ```
    ...
    <Accordion title="Show hook installer script">
      ...
      # model it keeps nudging toward pioneer/auto via X-Fastino-Router-Tip.
      # No-op when not on a Fastino gateway, when no signal is present, or when
      ...
      DEFAULT_ROUTER_TIP = (
          "Tip: use model=pioneer/auto to let Fastino route each request automatically; "
      ...
      if normalized_session == "pioneer/auto":
          ...
          if savings_total > 0:
              ...
              message = (
                  f"Fastino routing saved ~{format_usd(savings_total)} this session "
                  f"(vs {reference})"
              )
      elif routed:
  ...
  <Step title="Launch Claude Code">
    ...
    Use `/model` inside Claude Code to pick a specific Fastino model, or keep `pioneer/auto` to use the Code Router.
  </Step>
</Steps>

<Tip>
  The Fastino dashboard **Integrations** guide copies a single shell block that runs all of the steps above with your API key filled in.
</Tip>

## Use the Code Router (`pioneer/auto`)

`pioneer/auto` sends each turn through Fastino's Code Router. The router picks the cheapest model that meets your quality bar for that specific prompt.
...
The Fastino setup registers a Stop hook in `~/.claude/settings.json`. That script reads the `pioneer_savings` rate differences and the per-turn token usage from the session transcript, sums the savings across the session, and emits a short message. Claude Code labels hook output with the hook name, so the UI shows:

```text
Stop says: Fastino routing saved ~$1.43 this session (vs claude-opus-4-7)
```

This is informational. **Stop** is Claude Code's name for the end-of-turn hook — not a warning and not a command to halt. The amount is cumulative for the session and only appears once the Code Router has actually saved money versus the frontier reference model.

Fastino also includes `pioneer_routed_model` and `pioneer_savings` on Anthropic-compatible responses (streaming `message_start` frames and non-streaming bodies) if you want to build your own tooling around routing metadata.

## Full model list in `/model`

Claude Code only shows third-party gateway models whose IDs start with `claude` or `anthropic`. Fastino publishes discovery aliases such as `anthropic/pioneer/gpt-5.4` on `GET /v1/models` so non-Claude decoder models appear in the picker while still resolving to the canonical Fastino model ID at inference time.

Requirements:

- `CLAUDE_CODE_ENABLE_GATEWAY_MODEL_DISCOVERY=1`
- `ANTHROPIC_BASE_URL` pointing at Fastino
...
<AccordionGroup>
  <Accordion title="Claude Code says Not logged in · Please run /login" icon="circle-alert">
    This usually means Claude Code did not pick up a usable Fastino API key for the current shell. It is not asking you to sign in to Claude.ai.
    ...
    3. Run `/status` and confirm:
       ...
       - **Anthropic base URL** is your Fastino endpoint
       - **Auth token** is `none`
    4. If `/status` points at Fastino but the error persists, check `~/.claude.json`. Under `customApiKeyResponses`, make sure the suffix for your current Fastino key is in `approved`, not `rejected`.
    5. If you are using a non-production Fastino endpoint, make sure the API key comes from the same environment.
  </Accordion>

  <Accordion title="Claude.ai login conflicts with your Fastino key" icon="key-round">
    If you see `Both claude.ai and ANTHROPIC_API_KEY set · auth may not work as expected`, Claude Code is still using Claude.ai OAuth instead of your Fastino key.
    ...
    3. Verify with `/status`:
       ...
       - **Anthropic base URL** should be your Fastino endpoint
