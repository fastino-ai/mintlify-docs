...
<Steps>
  ...
  <Step title="Persist Pioneer env vars">
    Create a sourced env file so every new shell picks up the right gateway settings:

```bash
mkdir -p ~/.pioneer
cat > ~/.pioneer/env <<'EOF'
# Pioneer inference
# Clear higher-precedence Claude Code auth so the Pioneer key is used.
...
```
    ...
    Add it to your shell rc file if it is not already sourced:
...
```
  ...
  <Step title="Install the routed-model hook">
    When you use `pioneer/auto`, Pioneer returns `pioneer_routed_model` on each response. Claude Code does not display that field by default — install the Stop hook so each turn ends with a line like `Routed pioneer/auto → claude-opus-4-7`:
...
Install the Stop hook from the setup section. It only runs when `ANTHROPIC_BASE_URL` points at Pioneer (`pioneer.ai`, `localhost`, or `127.0.0.1`).

### Bedrock-related errors

Disable thinking mode when launching:
