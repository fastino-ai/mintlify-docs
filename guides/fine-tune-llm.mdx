---
title: "Fine-tune Nemotron 3.5 Lightning on Fastino"
sidebarTitle: "LLM Fine-tuning / Post-training"
description: "LoRA fine-tune Nemotron 3.5 Lightning on Fastino with supervised fine-tuning via one training endpoint — from dataset prep to a deployed decoder model."
hidden: true
---

Fastino supports parameter-efficient (LoRA) post-training on the Nemotron 3.5 Lightning family. You bring your training data, choose the general, finance, or healthcare target, and Fastino handles the infrastructure, routing, and serving. The result is a fine-tuned adapter you can call over the same API, with no GPU management required.

New decoder training jobs use supervised fine-tuning (SFT) through the [`POST /v1/felix/training-jobs`](/training) endpoint.
...
<Note>
  ...
  job's `id` as the `model` at inference time. `training_type` defaults to
...
<Steps>
  <Step title="Choose a decoder base model">
    Use `GET /v1/base-models` to see the full current catalog, filtered to models that support training:

    <CodeGroup>

    ```bash cURL
    curl "https://api.fastino.ai/v1/base-models?task_type=decoder&supports_training=true" \
  ...
  <Step title="Prepare your training data">
    ...
    See the [Synthetic Data guide](/guides/synthetic-data) for the full set of `/v1/generate` options, including auto-labelling existing text. Once generated or uploaded, wait until the dataset status is `ready` before starting training.
  </Step>

  <Step title="Start a training job">
    Submit your SFT job with `POST /v1/felix/training-jobs`:
    ...
    Fastino routes your job automatically to the best available provider. The response includes your job ID:
  ...
  <Step title="Poll until training is complete">
    Check job status by polling `GET /v1/felix/training-jobs/:id`.
    ...
    ```bash cURL
    curl https://api.fastino.ai/v1/felix/training-jobs/YOUR_JOB_ID/logs \
      -H "X-API-Key: YOUR_API_KEY"
    ```
  ...
  <Step title="Run inference on your fine-tuned model">
    Once the job status is `deployed`, use your job ID as the `model` on a
    supported inference interface.
...
<Note>
  Downloading your trained model weights is available on the Pro plan and above. Use `GET /v1/felix/training-jobs/:id/download` to retrieve the weights once training is complete.
</Note>
...
Use `GET /v1/base-models?supports_training=true` immediately before submitting a
...
Use `GET /v1/base-models?task_type=decoder&supports_inference=true` to see the current serverless catalog.
...
- [Training Jobs API](/training) — every endpoint, parameter, and response field
