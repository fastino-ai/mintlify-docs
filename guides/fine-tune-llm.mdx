...
New decoder training jobs use supervised fine-tuning (SFT) through the [`POST /v1/training-jobs`](/training) endpoint.
...
<Steps>
  ...
  <Step title="Start a training job">
    Submit your SFT job with `POST /v1/training-jobs`:
  ...
  <Step title="Poll until training is complete">
    Check job status by polling `GET /v1/training-jobs/:id`.
    ...
    ```bash cURL
    curl https://api.fastino.ai/v1/training-jobs/YOUR_JOB_ID/logs \
      -H "X-API-Key: YOUR_API_KEY"
    ```
...
<Note>
  Downloading your trained model weights is available on the Pro plan and above. Use `GET /v1/training-jobs/:id/download` to retrieve the weights once training is complete.
</Note>
