...
New decoder training jobs use supervised fine-tuning (SFT) through the [`POST /felix/training-jobs`](/training) endpoint.
...
<Steps>
  ...
  <Step title="Run inference on your fine-tuned model">
    ...
    **Fastino native API** — use `"task": "generate"` for decoder models:
