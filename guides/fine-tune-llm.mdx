...
<Steps>
  ...
  <Step title="Prepare your training data">
    ...
    See the [Synthetic Data guide](/guides/synthetic-data) for the full set of `/generate` options, including auto-labelling existing text. To upload your own chat JSONL, use `POST /felix/datasets/upload/url` with `dataset_type: "decoder"`. For a held-out eval set, also pass `type: "evaluation"` — see [Datasets](/concepts/datasets#uploading-your-own-dataset). Wait until the dataset status is `ready` before starting training.
  </Step>
