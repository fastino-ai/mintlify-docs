---
title: "Fine-tune a GLiNER structured extraction model on Fastino"
sidebarTitle: "Extraction Fine-tuning"
description: "Train a custom JSON extraction model on Fastino's GLiNER encoders to pull invoices, forms, and structured records out of unstructured text — data to inference."
hidden: true
---

Structured extraction pulls JSON-shaped records — invoices, contracts, product specs, medical reports, any form with named fields — out of unstructured text. You define a structure (a named bundle of fields), train on examples that fill it in, and the model learns to extract the same shape from new documents. Fastino's GLiNER encoder models handle extraction in the same forward pass they use for NER and classification, so you get one small, fast model that does all three.

<Steps>
  <Step title="Choose a base model">
    Fastino supports GLiNER2 training targets for general extraction workloads. For most tasks, `fastino/gliner2-base-v1` is the right starting point. If your documents include non-English text, use a `multi` variant instead.
    ...
    ```bash cURL
    curl "https://api.fastino.ai/v1/base-models?task_type=encoder&supports_training=true" \
  ...
  <Step title="Prepare your training data">
    Structured extraction needs labeled examples — Fastino's synthetic data generator covers NER, classification, and decoder tasks but does not generate structures, so plan on bringing your own labeled examples (typically dozens to a few hundred to start).
    ...
    **A few rules to keep in mind:**
    ...
    Once your dataset is uploaded through the Fastino dashboard, confirm its status before starting training:

    <CodeGroup>

    ```bash cURL
    curl https://api.fastino.ai/v1/felix/datasets/my-extraction-dataset \
      -H "X-API-Key: YOUR_API_KEY"
    ```
  ...
  <Step title="Start a training job">
    Submit your training job with `POST /v1/felix/training-jobs`. Set `base_model` to the GLiNER model you chose in step 1 and `training_type` to `"lora"`. The training endpoint is shared with NER and classification — Fastino infers the task heads from the dataset columns.
  ...
  <Step title="Poll job status and review metrics">
    ...
    ```bash cURL
    curl https://api.fastino.ai/v1/felix/training-jobs/YOUR_JOB_ID \
      -H "X-API-Key: YOUR_API_KEY"
    ```
  ...
  <Step title="Run an evaluation">
    Pin the held-out cases in a project evaluation suite, then run that same suite against
    the base model and your training job. See [Evaluations](/api-reference/evaluations) for
    the current suite and run endpoints. Inspect per-case field failures before deploying.
  </Step>
  <Step title="Run inference with your trained model">
    Use your job ID as the `model` to run predictions through
    `POST /v1/chat/completions`. Extraction lives under the `structures` key of the
    top-level `schema` field. See the [Inference API](/inference) for the full request and
    response contract.
    ...
    curl -X POST https://api.fastino.ai/v1/chat/completions \
      ...
      -d '{
        ...
        "messages": [
          {
            "role": "user",
            "content": "Invoice INV-2024-0042 from Acme Corp for $1,250.00 USD. Items: Widget x 10, Premium support, 1 month."
          }
        ],
        "schema": {
          "structures": {
            "invoice": [
              "vendor::str::company issuing the invoice",
              "invoice_number::str::invoice identifier",
              "amount::str::total amount due including currency symbol",
              "currency::str::three-letter currency code",
              "line_items::list::each listed invoice item"
            ]
          }
        },
        "threshold": 0.5
    ...
    **Field specifications**
    ...
    Define each field as `name::type::description`, using `str` for one value or `list` for
    multiple values.
    ...
    You can make the same request with the OpenAI SDK. Set `base_url` to
    `https://api.fastino.ai/v1` and pass Fastino fields via `extra_body`:
...
- **Write descriptions for ambiguous fields.** `date::str::date the invoice was issued, not the due date` is more precise than `date::str` when both dates appear in the document.
...
- [Adaptive Inference](/guides/adaptive-inference) — let Fastino retrain your extractor automatically on live traffic
