---
title: "Evaluate Pioneer models with Braintrust and Prime Intellect"
sidebarTitle: "Third-party evals"
description: "Point Braintrust or Prime Intellect at your Pioneer models. Both speak the OpenAI API, so you only change base_url and your API key — no adapters or proxies."
---

Pioneer serves an OpenAI-compatible API, so third-party eval tools work against your models without any adapter. Set `base_url` to `https://api.pioneer.ai/v1`, pass your Pioneer API key, and put a model ID in the `model` field. Everything else in your eval harness stays the same.

This guide covers [Braintrust](https://www.braintrust.dev) for experiment tracking and LLM-as-judge scoring, and [Prime Intellect](https://docs.primeintellect.ai) for environment-based benchmarks such as GSM8K.
...
Prime Intellect's `verifiers` library runs environment-based benchmarks and reports rewards. Use the `vf-eval` command, which is open source and needs no Prime account.
...
vf-eval gsm8k \
  ...
  --api-key-var PIONEER_API_KEY \
  --num-examples 20 \
  --save-results
```

Results are written to `./outputs/evals/`, with one JSON row per example containing the prompt, completion, reward, and token usage.

For a base model without a chat template, add `--api-client-type openai_completions` so the harness calls `/v1/completions` instead of `/v1/chat/completions`.

### Save the endpoint

To avoid repeating the flags, register Pioneer in `./configs/endpoints.toml`:
...
<Note>
  The `prime eval run` wrapper requires a Prime Intellect account key even when you target an external endpoint. `vf-eval` takes the same arguments and does not.
</Note>
...
<Accordion title="Token caps set on the verifiers CLI may not apply">
  Some `verifiers` environments override sampling arguments, so `--max-tokens` does not always reach the model. Pioneer honors `max_tokens` whenever it is sent, but if you are budgeting an eval run, check the reported token usage rather than assuming the flag capped it.
</Accordion>

<Accordion title="Braintrust --list needs an account key">
