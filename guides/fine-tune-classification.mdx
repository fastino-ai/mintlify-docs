...
<Steps>
  ...
  <Step title="Prepare your training data">
    ...
    **Option B — Auto-label existing text.** If you have raw text but no labels, send it to `POST /generate/classification/label-existing` and Fastino will annotate it synchronously. Accepts 1–1,000 strings per call.
  ...
  <Step title="Start a training job">
    Submit your training job with `POST /felix/training-jobs`. Set `base_model` to the GLiNER model you chose in step 1 and `training_type` to `"lora"`. The training endpoint is shared with NER — Fastino infers the task heads from the dataset columns.
  ...
  <Step title="Run inference with your trained model">
    ...
    **Classification entry options**
    ...
    You can also call inference using the OpenAI-compatible endpoint. Set `base_url` to `https://api.pioneer.ai/v1` and pass Fastino fields via `extra_body`:
