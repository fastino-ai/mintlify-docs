...
<Steps>
  ...
  <Step title="Prepare your training data">
    ...
    **Option C — Upload through the platform.** If you already have labeled data, upload it directly via the Fastino dashboard. Each row needs a `text` column and either a `label` column (single-label) or a `labels` column (multi-label).
    ...
    ```bash cURL
    curl https://api.fastino.ai/v1/datasets/my-classification-dataset \
      -H "X-API-Key: YOUR_API_KEY"
    ```
  ...
  <Step title="Start a training job">
    Submit your training job with `POST /v1/training-jobs`. Set `base_model` to the GLiNER model you chose in step 1 and `training_type` to `"lora"`. The training endpoint is shared with NER — Fastino infers the task heads from the dataset columns.
