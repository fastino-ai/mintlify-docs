---
title: "Pioneer API error codes: 400, 401, 402, 403, 404, 422, 429, 500"
sidebarTitle: "Errors"
description: "HTTP status codes returned by the Pioneer API — including 400, 401, 402, 403, 404, 409, 413, 422, 425, 429, 451, 500, and 503 — with plain-language explanations and steps to resolve each one."
---
...
Your account does not have sufficient credits to complete the request. The response body's `code` field tells you which case applies — most commonly `out_of_credits` (your included credits are exhausted and there's no spendable paid balance) or `direct_model_requires_credits` (included plan credits are router-only; calling a model directly instead of through `pioneer/auto` requires a paid credit balance).
