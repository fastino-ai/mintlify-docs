...
**How to fix:** Double-check the ID or name in the request path or body. Use the corresponding `GET` list endpoint (for example `GET /v1/felix/training-jobs`, `GET /v1/base-models`) to confirm the resource exists.
...
- Omitting `base_model` from `POST /v1/felix/training-jobs`
- Passing an unsupported `task_type` to `POST /v1/generate`
- Sending fewer than 1 or more than 1,000 strings in the `inputs` array for label-existing endpoints

```json
{
    "detail": "For 'POST /v1/felix/training-jobs', ...",
    "errors": [...]
}
```
