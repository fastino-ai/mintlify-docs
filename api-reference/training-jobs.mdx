...
<ParamField body="training_algorithm" type="string">
  Post-training algorithm. Accepted values: `sft` (default), `grpo`, `dpo`. `sft` is supervised fine-tuning; `grpo` and `dpo` are reinforcement-learning methods that require `rl_config`. See the [Post-training guide](/guides/post-training). Submitting `grpo`/`dpo` for a model that hasn't been verified for RL returns a `422`.
</ParamField>
