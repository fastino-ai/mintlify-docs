...
<ParamField body="dataset_name" type="string">
  The name of the labeled dataset to evaluate against. The dataset must be in the `ready` state. Upload a held-out set with `type: "evaluation"` (and `dataset_type: "decoder"` for chat JSONL) via `POST /felix/datasets/upload/url`. See [Datasets](/concepts/datasets#uploading-your-own-dataset).
</ParamField>
