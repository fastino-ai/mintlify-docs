---
title: "Fastino projects API: deploy and manage model endpoints"
sidebarTitle: "Projects"
description: "Organize Fastino models into projects, deploy fine-tuned models to project endpoints, and run inference without managing training job IDs in your application."
---
...
<Note>
  After deploying a model, use `POST /v1/projects/:project_id/inference` to run predictions through the project endpoint — no need to track training job IDs in your application code.
</Note>
...
`POST /v1/projects/:project_id/inference`
...
```bash
curl -X POST https://api.fastino.ai/v1/projects/YOUR_PROJECT_ID/inference \
  -H "X-API-Key: YOUR_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
