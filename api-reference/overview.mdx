...
Fastino exposes drop-in replacements for the OpenAI and Anthropic SDKs. Point your existing client at `https://api.pioneer.ai/v1` and use your Fastino API key — no other changes are required.

| SDK | Base URL | Notes |
| --- | --- | --- |
| OpenAI | `https://api.pioneer.ai/v1` | Set `base_url`; pass Fastino-specific fields via `extra_body` |
