...
Every request requires an `X-API-Key` header. Create your key in the Pioneer dashboard at [agent.pioneer.ai/api-keys](https://agent.pioneer.ai/api-keys), or open **Settings** -> **API Keys** after signing in. See the [Authentication](/api-reference/authentication) page for full details, including key management endpoints.
