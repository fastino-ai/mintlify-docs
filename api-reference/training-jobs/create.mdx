...
<ParamField body="base_model" type="string" required>
  A trainable model ID from `GET /base-models?supports_training=true`, or a supported checkpoint UUID.
</ParamField>
...
Returns `200` with the full training-job record. Save its `id` and poll [`GET /felix/training-jobs/{job_id}`](/api-reference/training-jobs/get).
