---
title: "Inference history and feedback endpoints on Fastino"
sidebarTitle: "History & Feedback"
description: "List Fastino inference history, filter by model or project, retrieve individual results, and submit corrections to improve your model via Adaptive Inference."
hidden: true
noindex: true
---
...
| `GET` | `/v1/inferences/:id` | Get inference details |
| `POST` | `/v1/inferences/:id/feedback` | Submit feedback |
| `GET` | `/v1/inferences/:id/feedback` | Get stored feedback |

## List past inferences

`GET /v1/inferences` returns a paginated list of past inference calls. Use the query parameters below to filter results.
...
`GET /v1/inferences/:id` returns the full record for a single past inference, including the input text, schema, model response, and timestamp.
...
`POST /v1/inferences/:id/feedback` lets you mark a past inference as correct or incorrect, optionally attaching the corrected output. Incorrect verdicts with a correction are used as labeled training examples for Adaptive Inference.

<Note>
  Feedback submitted here powers Adaptive Inference — Fastino's continuous improvement loop that automatically retrains your model on corrections collected from live traffic. See the [Adaptive Inference guide](/guides/adaptive-inference) for details on how this works.
</Note>
...
`GET /v1/inferences/:id/feedback` returns the feedback previously submitted for a specific inference. Returns `404` if no feedback has been submitted yet.
...
- [Fastino native inference](/api-reference/inference/pioneer) — run new inferences
