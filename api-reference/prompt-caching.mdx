...
description: "How prompt caching works on Fastino's inference API — what qualifies for caching, how cached input tokens are billed, and where to view cache hit rates."
noindex: true
---
...
- `POST /inference` (Fastino native, decoder generation) — send a message `content` as a content-block array and mark a block.
...
Most harnesses can emit this shape but need to be told to use Anthropic-style cache control for Fastino's Claude models. When configuring a custom Fastino provider, make sure the client is set to:
