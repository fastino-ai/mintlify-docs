...
Use `/model` to pick another supported Pioneer model. Follow the [Claude Code integration](/claude-code) guide for the full setup and model picker.
...
3. Register Pioneer as a custom provider
   - This example registers five Pioneer-backed decoder models. You can swap or add models using the IDs returned by the previous step.
     ...
     openclaw config set models.providers.pioneer '{
       ...
       "models": [
         { "id": "claude-sonnet-5",             "name": "Claude Sonnet 5",    "input": ["text"], "contextWindow": 1000000, "maxTokens": 16000 },
         { "id": "claude-opus-5",               "name": "Claude Opus 5",      "input": ["text"], "contextWindow": 1000000, "maxTokens": 16000 },
         { "id": "deepseek-ai/DeepSeek-V4-Flash","name": "DeepSeek V4 Flash", "input": ["text"], "contextWindow": 1000000, "maxTokens": 16000 },
         { "id": "zai-org/GLM-5.2",             "name": "GLM 5.2",            "input": ["text"], "contextWindow": 1048576, "maxTokens": 16000 },
         { "id": "gpt-5.5",                     "name": "GPT-5.5",            "input": ["text"], "contextWindow": 1050000, "maxTokens": 16000 }
       ]
     }' --strict-json
     ```
...
4. Expose the models to agents
   - Registering the provider adds models to OpenClaw's provider catalog. To make them visible to the default agent and model picker, add each model under `agents.defaults.models`.
     ```shellscript
     openclaw config set 'agents.defaults.models["pioneer/claude-sonnet-5"]'             '{"alias":"Pioneer Sonnet 5"}'        --strict-json
     openclaw config set 'agents.defaults.models["pioneer/claude-opus-5"]'               '{"alias":"Pioneer Opus 5"}'          --strict-json
     openclaw config set 'agents.defaults.models["pioneer/deepseek-ai/DeepSeek-V4-Flash"]' '{"alias":"Pioneer DeepSeek V4 Flash"}' --strict-json
     openclaw config set 'agents.defaults.models["pioneer/zai-org/GLM-5.2"]'             '{"alias":"Pioneer GLM 5.2"}'         --strict-json
   ...
   - Set the default model if needed:
     ```shellscript
     openclaw models set pioneer/claude-sonnet-5
     ```
...
6. Restart and verify
   ...
   - Expected result:
     ```text
     Auth probes
     pioneer/claude-sonnet-5 | pioneer:manual (api_key) | ok
     ```
