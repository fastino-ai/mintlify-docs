...
<Steps>
  ...
  <Step title="Inicia un trabajo de entrenamiento (opcional)">
    ...
    La respuesta incluye un `id` de trabajo. Consulta `GET /v1/felix/training-jobs/{id}` para comprobar el estado. Cuando el trabajo llegue a `complete`, usa el ID del trabajo como `model` en las llamadas a `/v1/chat/completions`.
