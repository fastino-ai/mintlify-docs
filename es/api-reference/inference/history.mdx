---
title: "Endpoints de historial de inferencia y feedback en Fastino"
sidebarTitle: "Historial y feedback"
description: "Lista el historial de inferencias de Fastino, filtra por modelo o proyecto y envía correcciones que Adaptive Inference usa para reentrenar tu modelo."
hidden: true
noindex: true
---

Fastino almacena cada llamada de inferencia y te permite recuperar resultados por ID o en bloque. También puedes enviar feedback de corrección sobre inferencias individuales. Este feedback señala en qué se equivocó el modelo e impulsa Adaptive Inference, que reentrena automáticamente tu modelo con ejemplos corregidos provenientes del tráfico real.

## Endpoints

| Method | Path | Descripción |
|---|---|---|
| `GET` | `/v1/inferences` | Lista inferencias pasadas |
| `GET` | `/v1/inferences/:id` | Obtiene los detalles de una inferencia |
| `POST` | `/v1/inferences/:id/feedback` | Envía feedback |
| `GET` | `/v1/inferences/:id/feedback` | Obtiene el feedback almacenado |

## Listar inferencias pasadas

`GET /v1/inferences` devuelve una lista paginada de llamadas de inferencia pasadas. Usa los parámetros de consulta siguientes para filtrar los resultados.
...
`GET /v1/inferences/:id` devuelve el registro completo de una única inferencia pasada, incluyendo el texto de entrada, el esquema, la respuesta del modelo y la marca de tiempo.
...
`POST /v1/inferences/:id/feedback` te permite marcar una inferencia pasada como correcta o incorrecta, adjuntando opcionalmente la salida corregida. Los veredictos incorrectos que incluyen una corrección se usan como ejemplos de entrenamiento etiquetados para Adaptive Inference.

<Note>
  El feedback enviado aquí impulsa Adaptive Inference, el ciclo de mejora continua de Fastino que reentrena automáticamente tu modelo con las correcciones recopiladas del tráfico real. Consulta la [guía de Adaptive Inference](/guides/adaptive-inference) para más detalles sobre cómo funciona.
</Note>
...
`GET /v1/inferences/:id/feedback` devuelve el feedback enviado previamente para una inferencia concreta. Devuelve `404` si todavía no se ha enviado ningún feedback.
...
- [Inferencia nativa de Fastino](/api-reference/inference/pioneer): ejecuta nuevas inferencias
