...
<ParamField body="base_model" type="string" required>
  Un ID de modelo entrenable de `GET /base-models?supports_training=true`, o un UUID de checkpoint soportado.
</ParamField>
...
Devuelve `200` con el registro completo del trabajo de entrenamiento. Guarda su `id` y consulta [`GET /felix/training-jobs/{job_id}`](/es/api-reference/training-jobs/get).
