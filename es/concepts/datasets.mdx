...
Solo `dataset_name` es obligatorio. `dataset_type` vale `"ner"` por defecto y acepta `"ner"`, `"classification"`, `"custom"` o `"decoder"` (JSONL de chat para LLMs decoder). El campo `type` es `"training"` por defecto (entrenable); usa `"evaluation"` para un conjunto de prueba de `POST /felix/evaluations` (no entrenable). `"benchmark"` se rechaza aquí (`403`) porque esos datasets los gestiona el sistema. `project_id` (UUID) es opcional. La respuesta incluye `presigned_url`, `dataset_id` y `version_number`.
