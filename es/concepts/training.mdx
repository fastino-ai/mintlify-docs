---
title: "Trabajos de entrenamiento Fastino: ciclo de vida y pesos"
sidebarTitle: "Entrenamiento"
description: "Cómo funcionan los trabajos de entrenamiento de Fastino: envía un trabajo, sondea su estado, lee métricas, detén trabajos y descarga los pesos entrenados."
---

El fine-tuning en Fastino adapta un modelo base a tu tarea y dominio específicos usando tu dataset etiquetado. Envías un trabajo de entrenamiento a través de la API, Fastino gestiona la computación y tú recibes un modelo entrenado al que puedes llamar para inferencia o descargar. Todo el proceso es asíncrono: inicias el trabajo y luego sondeas hasta que termina.

Los nuevos trabajos de entrenamiento de Fastino usan fine-tuning supervisado (SFT). Consulta la [guía de fine-tuning de LLM](/guides/fine-tune-llm) para conocer el formato de los datasets y ver ejemplos de entrenamiento de decoders.
...
<Steps>
  <Step title="requested">
    Tu trabajo se ha aceptado y está en cola para ejecutarse. Fastino está asignando cómputo.
  </Step>
  <Step title="running">
  ...
  <Step title="normalizing / artifact_ready">
    Pasos intermedios de post-entrenamiento. Fastino está normalizando y empaquetando el artefacto entrenado. Normalmente solo verás estos estados de forma transitoria entre `complete` y `deployed`.
  </Step>
  <Step title="deployed">
...
| `base_model` | Sí | El ID del modelo a afinar. Usa un valor de `GET /v1/base-models` o un UUID de checkpoint de un trabajo anterior. |
...
## Destinos de entrenamiento compatibles

Los nuevos trabajos de entrenamiento solo admiten las siguientes familias de destino:

| Familia | Destinos compatibles |
| --- | --- |
| Encoders GLiNER | GLiNER2.5 Multi |

Usa `GET /v1/base-models?supports_training=true` justo antes de crear un
trabajo. Es la fuente de verdad en tiempo real sobre la disponibilidad de destinos.
...
Fastino guarda checkpoints durante el entrenamiento. Puedes listarlos en cualquier momento tras iniciar el trabajo:
...
|---|---|---|
| `POST` | `/v1/training-jobs` | Inicia un nuevo trabajo de entrenamiento |
| `GET` | `/v1/training-jobs` | Lista trabajos de entrenamiento (filtra por `project_id`, `status`; pagina con `limit`/`offset`) |
| `GET` | `/v1/training-jobs/:id` | Obtiene el estado y las métricas del trabajo |
| `GET` | `/v1/training-jobs/:id/logs` | Obtiene entradas estructuradas del log de entrenamiento |
| `GET` | `/v1/training-jobs/:id/checkpoints` | Lista los checkpoints guardados |
| `POST` | `/v1/training-jobs/:id/checkpoints/:checkpoint_id/deploy` | Despliega un checkpoint concreto para inferencia |
| `GET` | `/v1/training-jobs/:id/download` | Obtiene una URL prefirmada para descargar los pesos entrenados (plan Pro o superior) |
| `POST` | `/v1/training-jobs/:id/stop` | Detiene de forma controlada un trabajo en ejecución preservando los checkpoints |
| `POST` | `/v1/training-jobs/:id/terminate` | Detiene el trabajo y elimina permanentemente sus checkpoints (irreversible) |
| `DELETE` | `/v1/training-jobs/:id` | Elimina el registro del trabajo. También lo detiene si está activo y borra sus checkpoints |
