...
|---|---|---|
| `POST` | `/v1/felix/training-jobs` | Inicia un nuevo trabajo de entrenamiento |
| `GET` | `/v1/felix/training-jobs` | Lista trabajos de entrenamiento (filtra por `project_id`, `status`; pagina con `limit`/`offset`) |
| `GET` | `/v1/felix/training-jobs/:id` | Obtiene el estado y las métricas del trabajo |
| `GET` | `/v1/felix/training-jobs/:id/logs` | Obtiene entradas estructuradas del log de entrenamiento |
| `GET` | `/v1/felix/training-jobs/:id/checkpoints` | Lista los checkpoints guardados |
| `POST` | `/v1/felix/training-jobs/:id/checkpoints/:checkpoint_id/deploy` | Despliega un checkpoint concreto para inferencia |
| `GET` | `/v1/felix/training-jobs/:id/download` | Obtiene una URL prefirmada para descargar los pesos entrenados (plan Pro o superior) |
| `POST` | `/v1/felix/training-jobs/:id/stop` | Detiene de forma controlada un trabajo en ejecución preservando los checkpoints |
| `POST` | `/v1/felix/training-jobs/:id/terminate` | Detiene el trabajo y elimina permanentemente sus checkpoints (irreversible) |
| `DELETE` | `/v1/felix/training-jobs/:id` | Elimina el registro del trabajo. También lo detiene si está activo y borra sus checkpoints |
