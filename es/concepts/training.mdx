---
title: "Trabajos de entrenamiento en Pioneer: ciclo de vida, métricas y pesos"
sidebarTitle: "Entrenamiento"
description: "Entiende cómo funcionan los trabajos de entrenamiento de Pioneer: desde enviar un trabajo y sondear su estado hasta leer métricas, detener trabajos y descargar los pesos del modelo entrenado."
---
...
Pioneer admite tres algoritmos de post-entrenamiento, que se seleccionan con el parámetro `training_algorithm`: fine-tuning supervisado (`sft`, el predeterminado) y los métodos de aprendizaje por refuerzo GRPO (`grpo`) y DPO (`dpo`). Consulta la [guía de fine-tuning de LLM](/guides/fine-tune-llm) para saber cuándo usar cada uno y cómo dar formato a los datasets.
...
| `training_algorithm` | No | `"sft"` (predeterminado), `"grpo"` o `"dpo"`. Los algoritmos de RL requieren un `rl_config`. Consulta [fine-tuning de LLM](/guides/fine-tune-llm). |
