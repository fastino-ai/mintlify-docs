...
<Warning>
  El endpoint heredado `POST /v1/completions` se ha eliminado. Migra a `POST /v1/chat/completions` y envía el prompt como un mensaje de usuario.
</Warning>
...
Fastino también expone un endpoint compatible con Anthropic. Configura el `base_url` de tu SDK como `https://api.fastino.ai` y usa tu API key de Fastino en lugar de una key de Anthropic. El SDK añade `/v1/messages` y envía la clave como `x-api-key`.
