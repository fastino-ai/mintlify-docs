...
Fastino expone dos operaciones de inferencia GLiNER publicadas:

- `POST /v1/chat/completions` utiliza un envoltorio de solicitud compatible con OpenAI más
  la extensión `schema` de nivel superior de Fastino.
- `POST /v1/gliner-2` proporciona el contrato nativo de solicitud y respuesta de GLiNER
  publicado en el [documento OpenAPI](https://api.fastino.ai/openapi.json).

Esta página documenta el contrato HTTP en crudo de `/v1/chat/completions`. No cubre el uso del
SDK, el entrenamiento de modelos, la evaluación, el historial de inferencia ni el feedback.
