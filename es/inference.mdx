...
- `POST /v1/chat/completions` usa una solicitud compatible con OpenAI más la extensión
  `schema` de nivel superior de Fastino.
- `POST /v1/gliner-2` ofrece el contrato nativo de GLiNER publicado en el
  [documento OpenAPI](https://api.fastino.ai/openapi.json).

Esta página documenta el contrato HTTP en crudo de `/v1/chat/completions`. No cubre el uso
del SDK, el entrenamiento de modelos, la evaluación, el historial de inferencia ni el feedback.
