...
4. `No target session selected`
   - Pass a session target:
     ...
     openclaw agent --agent main --session-key cli-test --message "hello"
     ```
5. `gateway already running` when using `openclaw gateway run`
   - The LaunchAgent service is already bound to the gateway port. Use the Web UI or restart/stop the service instead:
     ```shellscript
     openclaw dashboard
     openclaw gateway restart
     # or, for foreground debugging only:
