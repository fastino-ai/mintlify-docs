...
4. Safely install or restart the gateway
   ...
   - Configure gateway token auth before checking the dashboard or gateway probe. This avoids the Control UI getting stuck at `Auth required` and avoids `device identity required` during status probes:
     ```shellscript
     openclaw config set gateway.mode local

     if [ -z "$(openclaw config get gateway.auth.token 2>/dev/null)" ]; then
     ...
     ```
   - If OpenClaw still reports that the service config is out of date or non-standard, repair the LaunchAgent and restart it:
     ```shellscript
...
6. Open the OpenClaw Web UI
   ...
   - If the gateway is already running as a service, do not start a second foreground gateway with `openclaw gateway run`. Use the dashboard command or restart the service instead:
     ...
     openclaw dashboard
     ```
   - The dashboard command is the supported way to open the Web UI. It may print a clean URL such as `http://127.0.0.1:18789/` while copying a token-authenticated URL to your clipboard. If the browser opens the `Auth required` screen, paste the configured gateway token into the `Gateway Token` field:
     ```shellscript
     openclaw config get gateway.auth.token
     ```
   - Treat the gateway token as a local secret. Do not paste it into docs, tickets, screenshots, or shell logs.
   - If no token is configured, generate one and reopen the dashboard:
     ```shellscript
     openclaw doctor --generate-gateway-token --yes
     openclaw gateway restart
     openclaw dashboard
...
6. Dashboard says `Auth required`, `device identity required`, or `gateway token mismatch`
   ...
   - If the command is empty, generate a token and restart the gateway:
     ```shellscript
     openclaw doctor --generate-gateway-token --yes
     openclaw gateway restart
     openclaw dashboard
