...
<Note>
  **Prompt caching on Claude models is opt-in.** Pioneer never adds cache markers for you, and OpenClaw's custom provider uses the `openai-completions` API with its own prompt layout, so Claude prompt caching may not apply unless the client emits `cache_control` markers. OpenAI/GPT models cache automatically. See [Prompt Caching](/api-reference/prompt-caching).
</Note>