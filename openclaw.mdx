---
title: "Set up Pioneer as an OpenClaw custom provider"
...
1. `Missing auth - pioneer`
   - OpenClaw does not have a Pioneer auth profile. Run:
     ...
     openclaw models status --probe --probe-provider pioneer
     ```
2. Only `pioneer/auto` appears in OpenClaw
   ...
   - Confirm the catalog exposes the expected Pioneer router models before rerunning the discovery command:
     ...
     ```
   - Expected output includes `pioneer/auto`, `pioneer/auto_v1.1`, `pioneer/auto_v1.2`, and `pioneer/general`. If that works, rerun the discovery command and restart the gateway.
3. `Missing env var "PIONEER_API_KEY"`
...
4. `Gateway service not loaded`
   - Reinstall and restart the LaunchAgent:
     ...
     ```
5. `No target session selected`
   - Pass a session target:
     ```shellscript
     openclaw agent --agent main --session-key cli-test --message "hello"
     ```
6. `gateway already running` or `Port 18789 is already in use`
   ...
   - For foreground debugging only, stop the service first, then run the gateway in the foreground:
     ...
     ```
7. Dashboard says `Auth required`, `device identity required`, or `gateway token mismatch`
...
8. Gateway is running but probe fails
