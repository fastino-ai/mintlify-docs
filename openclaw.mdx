...
6. Open the OpenClaw Web UI
   ...
   - If the gateway is already running as a service, do not start a second foreground gateway with `openclaw gateway run`. Use the dashboard command or restart the service instead:
     ...
     openclaw dashboard
     ```
   - The dashboard command is the supported way to open the Web UI. In current OpenClaw versions it may print only the loopback URL, for example `http://127.0.0.1:18789/`; there may not be a separate tokenized URL to copy. Gateway/browser credentials are handled by OpenClaw's local device identity or token state.
   - If the browser asks for auth, or the CLI reports `device identity required` / `gateway token mismatch`, repair local gateway auth and reopen the dashboard:
     ```shellscript
     openclaw doctor --repair
     openclaw gateway restart
     openclaw dashboard
...
5. `gateway already running` or `Port 18789 is already in use`
   ...
   - For foreground debugging only, stop the service first, then run the gateway in the foreground:
     ...
     ```
6. Dashboard or probe says `device identity required` or `gateway token mismatch`
   - Local OpenClaw installs may use device identity or gateway token state instead of a tokenized dashboard URL. Do not manually extract tokens from OpenClaw's state database. Repair local gateway auth and reopen the dashboard:
     ```shellscript
     openclaw doctor --repair
     openclaw gateway restart
     openclaw dashboard
