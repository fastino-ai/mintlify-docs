---
title: "OpenClaw"
description: "Configure OpenClaw to use Fastino's OpenAI-compatible endpoint, discover models via /v1/models, and run the local gateway with Fastino Auto."
hidden: true
---

Fastino exposes an OpenAI-compatible inference endpoint, so OpenClaw can use Fastino as a custom provider today. This guide covers the custom-provider setup until Fastino is available as an official OpenClaw provider.
...
- A Fastino API key from the Fastino dashboard.
- `jq` installed for converting the live Fastino model catalog into OpenClaw config.
...
1. Save Fastino auth in OpenClaw
   - Paste your Fastino API key once. OpenClaw stores it in the local auth profile store and the gateway reuses it across terminal sessions.
     ```shellscript
...
2. Discover and register Fastino models
   - Fastino's supported inference models can change over time. Use the OpenAI-compatible `GET /v1/models` catalog as the source of truth and convert it directly into OpenClaw model config.
   - This command reads the Fastino key from the OpenClaw auth profile saved in step 1, uses it for the catalog fetch, and writes the discovered model catalog into `~/.openclaw/openclaw.json`.
     ```shellscript
     AUTH_DB="$(openclaw models auth list --provider pioneer --json | jq -r '.authStatePath')"
     case "$AUTH_DB" in
       "~/"*) AUTH_DB="$HOME/${AUTH_DB#"~/"}" ;;
     esac
     FASTINO_API_KEY="$(sqlite3 "$AUTH_DB" "select json_extract(store_json, '$.profiles.\"pioneer:manual\".key') from auth_profile_store where store_key = 'primary';")"
     : "${FASTINO_API_KEY:?Run openclaw models auth paste-api-key --provider pioneer first}"
     CONFIG_FILE="$(openclaw config file)"
     case "$CONFIG_FILE" in
       "~/"*) CONFIG_FILE="$HOME/${CONFIG_FILE#"~/"}" ;;
     esac
     [ -f "$CONFIG_FILE" ] || printf '{}\n' > "$CONFIG_FILE"
     curl -fsS "https://api.fastino.ai/v1/models" \
     ...
     ```
   - The command manually adds `pioneer/auto` for Fastino Auto, then reads the top-level `.models[]` catalog when present and falls back to `.data[]`, deduplicates by `id`, filters `anthropic/*` Claude Code discovery aliases so OpenClaw does not show duplicate models, and exposes every Fastino model under `agents.defaults.models` for the model picker and `openclaw models status`.
   - OpenClaw uses `/think` for reasoning controls. Models that advertise `supported_reasoning_levels`, `reasoning`, `supports_reasoning`, or `thinking` metadata are registered with `reasoning: true`; when Fastino advertises exact reasoning levels, the command also writes `compat.supportedReasoningEfforts` so OpenClaw can include levels such as `xhigh`.
   - To refresh the catalog later, re-run the same command. It replaces the Fastino provider models and Fastino agent allowlist while preserving non-Fastino agent model entries.
3. Start the local gateway and open the Web UI
   ...
   - Use the LaunchAgent service for normal local setup. Do not run `openclaw gateway run` unless you are intentionally debugging in the foreground.
     ...
     unset GATEWAY_TOKEN
     ```
   - This avoids the noisy full `openclaw doctor` flow during normal setup. `openclaw gateway install --force` keeps the macOS LaunchAgent service definition current, including the service environment. `openclaw gateway restart` then applies the Fastino model config and gateway token.
   ...
   - In the Web UI, choose a reasoning-capable Fastino model and use the thinking selector to switch levels. From the CLI or chat input, send `/think low`, `/think medium`, `/think high`, or `/think off`. Send `/think` with no argument to see the current effective level.
4. Verify the setup **(optional)**
   - These checks are useful when validating a fresh setup or debugging a user report:
     ...
     ```
   - Expected result: `defaultModel` is `pioneer/auto`, `allowed_count` is greater than `1`, and the Fastino auth probe succeeds.
   - OpenClaw may probe only the default/effective target even when the agent allowlist contains many Fastino models. That is fine as long as the configured model count is greater than `1` and `pioneer/auto` probes successfully.
...
<AccordionGroup>
  <Accordion title="Missing auth - pioneer" icon="key-round">
    OpenClaw does not have a Fastino auth profile. Run:
  ...
  <Accordion title="Only pioneer/auto appears in OpenClaw" icon="list-filter">
    The catalog fetch likely did not read the saved OpenClaw auth profile. Confirm the Fastino profile exists, then rerun the discovery command.
    ...
    Confirm the catalog exposes the expected Fastino router models before rerunning the discovery command:
  ...
  <Accordion title='Missing env var "FASTINO_API_KEY"' icon="terminal">
    ...
    ```shellscript
    test -n "$FASTINO_API_KEY" && echo "set" || echo "not set"
    ```
