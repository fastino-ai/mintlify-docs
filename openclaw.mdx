...
3. Check the discovered models
   - Confirm OpenClaw sees the generated Pioneer provider catalog:
     ...
     ```
   - Confirm the default agent allowlist also contains the Pioneer models. `models status` reads `agents.defaults.models`, so it should show more than just `pioneer/auto` after the setup command:
     ```shellscript
...
5. Restart and verify
   - Restart the gateway so provider and auth changes are loaded:
     ...
     openclaw gateway status
     ```
   - Use a live probe, not just `openclaw models status`, to confirm Pioneer auth and model routing actually work. Because the setup command exposes the full Pioneer catalog to the default agent, this can probe many models:
     ```shellscript
