---
title: "CLI installation"
description: "Install the Fastino CLI on macOS or Linux, authenticate with your API key, and verify the setup by running your first command in the terminal."
sidebarTitle: "CLI installation"
noindex: true
hidden: true
---

Install the Fastino CLI when you want terminal access to training jobs, model artifacts, datasets, and the interactive Fastino agent.

<Info>
  Fastino CLI is published as `@fastino-ai/pioneer-cli` and runs with Bun `1.1.0` or newer.
</Info>

## Requirements

- macOS or Linux
- Bun `1.1.0` or newer
- Node.js and npm
- A Fastino API key from [Fastino API keys](https://agent.fastino.ai/api-keys)

## Install the CLI

<Steps>
  <Step title="Install Bun">
    ```bash
    curl -fsSL https://bun.sh/install | bash
    ```

    Open a new terminal after the installer finishes, then confirm Bun is available:

    ```bash
    bun --version
    ```
  </Step>
  <Step title="Install Fastino CLI">
    ```bash
    npm install -g @fastino-ai/pioneer-cli
    ```
  </Step>
  <Step title="Verify the command">
    ```bash
    pioneer --version
    pioneer --help
    ```
  </Step>
</Steps>

## Authenticate

<Steps>
  <Step title="Create an API key">
    Create an API key in the Fastino dashboard, then keep it ready for the login prompt.
  </Step>
  <Step title="Log in interactively">
    ```bash
    pioneer auth login
    ```

    The CLI validates the key before saving it.
  </Step>
  <Step title="Confirm your session">
    ```bash
    pioneer auth status
    ```
  </Step>
</Steps>

<Note>
  Interactive login stores your API key in `~/.pioneer/config.json`. The CLI also supports `PIONEER_API_KEY` for CI or short-lived shell sessions.
</Note>

## Check the setup

Run a few read-only commands to confirm the CLI can reach Fastino:

```bash
pioneer --version
pioneer auth status
pioneer model base-models
pioneer dataset list
```

## Update

```bash
npm install -g @fastino-ai/pioneer-cli@latest
pioneer --version
```

## Troubleshooting

<AccordionGroup>
  <Accordion title="pioneer: command not found" icon="circle-alert">
    Make sure your global npm bin directory is on `PATH`.

    ```bash
    npm bin -g
    ```
  </Accordion>

  <Accordion title="env: bun: No such file or directory" icon="terminal">
    Install Bun and open a new terminal:

    ```bash
    curl -fsSL https://bun.sh/install | bash
    bun --version
    ```
  </Accordion>

  <Accordion title="Authentication fails" icon="key-round">
    Create a new API key in the Fastino dashboard, then run interactive login again.

    ```bash
    pioneer auth login
    pioneer auth status
    ```
  </Accordion>
</AccordionGroup>
