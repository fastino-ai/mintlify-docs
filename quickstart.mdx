---
title: "Fastino quickstart: from signup to your first inference"
description: "Go from zero to a working Fastino inference call in minutes. Generate an API key, browse available models, and run your first NER prediction."
...
<Steps>
  ...
  <Step title="List available base models">
    Before running inference, you can browse the models Fastino has available. Use `GET /base-models` to see the full catalog. Pass `?supports_inference=true` to filter to models you can call immediately, or `?task_type=decoder` to see LLMs only.
...
<CardGroup cols={2}>
  ...
  <Card title="Fine-tune an LLM" icon="brain" href="/guides/fine-tune-llm">
    Fine-tune Nemotron 3.5 Lightning on your domain data with LoRA.
  </Card>
